...
import { formatCurrency, getStatus } from '../../utils/bill'
...
import { getBillsList, removeBillList } from '../../services/bill'
...
const ViewBills = ({ navigation }) => {
  ...
  const [translate] = useTranslation(SCREEN_KEYS.VIEW_BILLS)
  ...
  const animations = useFadeExpandAnimation({ isVisible: !!selectedId, expandedHeight: 133 })
  ...
  const handleReuseElementList = (item) => {
    ...
    navigation.replace(SCREEN_KEYS.NEW_BILL, {
  ...
  const handleRemoveElementList = async (elementListId) => {
    resetActionModal()
    await removeBillList(elementListId)
            ...
            <View style={styles.body}>
              <View style={styles.values}>
                <View style={styles.data}>
                  <CustomText text={`${translate('fields.amount')}:`} color={colors.text} />
                  <CustomText text={item.amount} color={colors.text} />
                </View>

                <View style={styles.data}>
                  <CustomText text={`${translate('fields.status')}:`} color={colors.text} />
                  <CustomText text={getStatus(item.status, statusList)} color={colors.text} />
                </View>
              </View>
            </View>
          </Reanimated.View>
